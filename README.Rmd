...
Using a dataframe with many properties

```{r}
tree <- add_node_properties(parse_newick("(mammal,(turtle,rayfinfish,(frog,salamander)))"),
                    data.frame(a = c("turtle","turtle","rayfinfish","frog"),
                               b = c("size","color","size","size"),
                               c = c("big","red","small","tiny"),
...
```{r}
tree
n1  <- tree$getNode("turtle")
nd1 <- n1$getNodeData()
nvd <- nd1$getNodeVisualData()

bd     <- n1$getBranchData()
color  <- J("java.awt.Color")
bcolor <- .jnew("org.forester.phylogeny.data.BranchColor", red)
red    <- color$RED
bd$setBranchColor(bcolor)

archaeopteryx(treestring=tree_to_XML(tree))
```
...
#n1  <- phyJtree$getNode("turtle")
#n1$getName()
#phyJ  <- set_clade_color(phyJ, "Node3", "test")
mrca  <- get_MRCA(phyJ, tips=c("frog", "turtle"))

color   <- J("java.awt.Color")
red      <- color$RED

phyJ  <- set_clade_color(phyJ, mrca, red)

treestring <- write_phyloXML(phyJ@phytreeJ)
archaeopteryx(treestring = treestring)
...
```{r}
iter1 <- J("org.forester.phylogeny.iterators.LevelOrderTreeIterator")
iter2 <- new(iter1, node1)
iter2$hasNext()
iter2$getNext()


## this is how you iterate over Jva objects from R
n3 <- .jrcall(iter2,"next");
n3
...
.jcall(iter2,  "org/forester/phylogeny/PhylogenyNode", "next")
.jcall(iter2,  "[org/forester/phylogeny/PhylogenyNode", "next")
.jcall(iter2,  "Ljava.lang.Object", "next")
.jcall(iter2,  "S", "toString")
.jcall(iter2,  "V", "remove")
?.jcall
