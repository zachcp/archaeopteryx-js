...
.jmethods(trtdata)
trtdata$addSequence(new(ss, new(ms)))

trtdata$addSeq

ss  <- J("org.forester.phylogeny.data.Sequence")
ms  <- J("org.forester.sequence.MolecularSequence")
msn  <- new(ms)
.jmethods(ms)
bs  <- J("org.forester.sequence.BasicSequence")
dna <- ms$TYPE$DNA
basic_sequence <- new(bs, "newstring", "AAACACACC", dna)
basic_sequence
.jclass(baseenv())
new(ss, new(ms))

trtdata$addSequence(basic_sequence)
.jmethods(trtdata)
J("org.forester.phylogeny.data.NodeData")
